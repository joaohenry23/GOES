...
class GOES():
    ...
    def __str__(self):
        ...
        for key in self.__dict__.keys():
            ...
            if isinstance(self.__dict__[key],np.ndarray):
                if self.__dict__[key].ndim == 0:
                    var.append('   {:29} {}'.format(key, str(self.__dict__[key])))#, str(self.__dict__[key].dtype)))
                elif self.__dict__[key].ndim == 1:
                    var.append('   {:29} ({}) {}'.format(key, str(self.__dict__[key].shape[0]), str(self.__dict__[key].dtype)))
                else:
                    var.append('   {:29} {} {}'.format(key, str(self.__dict__[key].shape), str(self.__dict__[key].dtype)))
    ...
    def refl_fact_to_refl(self, Lons, Lats, MinCosTheta=0.0, fmt=np.float32):
        ...
        MinCosTheta : float, optional
            Minimum valid value of the cosine of theta.

        fmt : dtype, optional
            The type of the returns. Default value is np.float32.
        ...
        if self.long_name == 'ABI L2+ Cloud and Moisture Imagery reflectance factor':
            ...
            try:
                ...
                dict_Field = {'data':self.data/cosine_of_solar_zenith_angle(Lons.data, Lats.data, self.t.data[0], MinCosTheta=MinCosTheta, fmt=fmt).data,
                              'long_name':'ABI L2+ Cloud and Moisture Imagery reflectance', 'standard_name':'toa_lambertian_equivalent_albedo',
                              'units':1, 'undef':np.nan, 'axis':'YX', 't':self.t, 'time_bounds':self.time_bounds, 'pixels_limits':self.pixels_limits}
...
class open_dataset():
    ...
    def get_attributes(self, parameter):
        '''
    ...
    def get_dimensions(self, parameter):
        '''

        It gets dimensions from file.
    ...
    def get_groups(self, parameter):
        '''

        It gets groups from file.
    ...
    def get_variables(self, parameter):
        '''

        It gets variables from file.
    ...
    def get_imagery(self, parameter, lonlat='center', domain=None, pixels=None, up_level=False, delta_index=4, fmt=np.float32):

        """

        It gets imagery from dataset with its longitude and latitude information.
        ...
        pixels : list or None, optional
            List with the pixels index of domain that you want to select.
            Example:
        ...
        up_level : boolean, optional
            ...
            brightness temperature. Default value is False.
        ...
        fmt : dtype, optional
            The type of the returns (Field, Lons and Lats). Default value is np.float32
        ...
        try:
            ...
            print('\n\tParameter not found, check if it is an imagery\n')
            return
        else:
            ...
            try:
                ...
                dict_Field = {'data':None,
                              'long_name':None, 'standard_name':None,
                              'units':None, 'undef':np.nan, 'pixels_limits':None, 'axis':'YX',
                              't':None, 'time_bounds':None}

                dict_Field['t'] = self.get_variables('t')
                dict_Field['time_bounds'] = self.get_variables('time_bounds')
                ...
                if isinstance(domain, list):
                    ...
                    xpixmin, xpixmax, ypixmin, ypixmax = pixels
    ...
    def __str__(self):
        ...
        for item in ds.variables.keys():
            if ds.variables[item].ndim == 0:
                var0D.append('   {:50} {}'.format(item, ds.variables[item].dtype))

        for item in ds.variables.keys():
            if ds.variables[item].ndim == 1:
                size = '({})'.format(ds.variables[item].dimensions[0])
                var1D.append('   {:50} {} {}'.format(item, size, ds.variables[item].dtype))

        for item in ds.variables.keys():
            if ds.variables[item].ndim == 2:
                size = '({}, {})'.format(ds.variables[item].dimensions[0],ds.variables[item].dimensions[1])
                var2D.append('   {:50} {} {}'.format(item, size, ds.variables[item].dtype))

        for item in ds.groups.keys():
            groups.append('   {:50}'.format(item))

        return '\n'.join([str(self.__class__)]+['']+attr+['']+dims+['']+var0D+var1D+['']+var2D+['']+group+[''])
...
class open_mfdataset():
    ...
    def get_dimensions(self, parameter):
        ...
        return GOES({'name':mfparameter_name, 'size':mfparameter_size});
    ...
    def __str__(self):
        ...
        for item in ds.variables.keys():
            if ds.variables[item].ndim == 0:
                var0D.append('   {:50}'.format(item))

        for item in ds.variables.keys():
            if ds.variables[item].ndim == 1:
                var1D.append('   {:50}'.format(item))

        for item in ds.variables.keys():
            if ds.variables[item].ndim == 2:
                var2D.append('   {:50}'.format(item))

        for item in ds.groups.keys():
            groups.append('   {:50}'.format(item))

        return '\n'.join([str(self.__class__)]+['']+nf+files+['']+attr+['']+dims+['']+var0D+var1D+['']+var2D+['']+group+[''])
...
def get_lonlat(X, Y, PlatformID, SatLon, SatHeight, SatSweep, fmt=np.float32):
    ...
    dict_Lons = {'data':Lons,
                 'long_name':'Longitude of center of pixels', 'standard_name':'pixels center longitude',
                 'units':'degrees_east', 'undef':-999.99, 'axis':'YX'}

    dict_Lats = {'data':Lats,
                 'long_name':'Latitude of center of pixels', 'standard_name':'pixels center latitude',
                 'units':'degrees_north', 'undef':-999.99, 'axis':'YX'}
...
def get_lonlatcorner(X, Y, PlatformID, SatLon, SatHeight, SatSweep, fmt=np.float32):
    ...
    dict_Lons = {'data':Lons,
                 'long_name':'Longitude of corners of pixels', 'standard_name':'pixels corners longitude',
                 'units':'degrees_east', 'undef':-999.99, 'axis':'YX'}

    dict_Lats = {'data':Lats,
                 'long_name':'Latitude of corners of pixels', 'standard_name':'pixels corners latitude',
                 'units':'degrees_north', 'undef':-999.99, 'axis':'YX'}
...
def cosine_of_solar_zenith_angle(Lons, Lats, DateTime, MinCosTheta=0.0, fmt=np.float32):
    ...
    try:
        ...
        dict_CosTheta = {'data':CosTheta,
                         'long_name':'cosine of solar zenith angle', 'standard_name':'cosine of solar zenith angle',
                         'units':None, 'undef':np.nan, 'axis':'YX'}
...
def create_gridmap(Domain, PixResol=2.0, fmt=np.float32):
    ...
    It creates a equirectangular gribmap and return theirs corners.

    Parameters
    ----------
    Domain : list
        List with the corners coordinates of domain.
        The list have four elements: [LeftLon, RightLon, LowerLat, UpperLat]

    PixResol : float, optional
    ...
    -------
    #LonsCen : object
    #    GOES class with gridmap center longitude.

    #LatsCen : object
    #    GOES class with gridmap center latitude.

    LonsCor : object
        GOES class with gridmap corner longitude.

    LatsCor : object
        GOES class with gridmap corner latitude.

    '''

    #'''
    #dict_LonsCen = {'data':None,
    #                'long_name':'Longitude of center of pixels', 'standard_name':'pixels center longitude',
    #                'units':'degrees_east', 'undef':-999.99, 'axis':'YX'}
    #dict_LatsCen = {'data':None,
    #                'long_name':'Latitude of center of pixels', 'standard_name':'pixels center latitude',
    #                'units':'degrees_north', 'undef':-999.99, 'axis':'YX'}
    #nx = int( np.ceil( (Domain[1] - Domain[0] -PixResol/111.0)*111.0/float(PixResol) ) )
    #ny = int( np.ceil( (Domain[3] - Domain[2] -PixResol/111.0)*111.0/float(PixResol) ) )
    #Lons = np.arange(nx+1,dtype=fmt)*(float(PixResol)/111.0)+Domain[0]+PixResol/(2.0*111.0)
    #Lats = np.arange(ny+1,dtype=fmt)*(float(PixResol)/111.0)*(-1.0)+Domain[3]-PixResol/(2.0*111.0)
    #Lons, Lats = np.meshgrid(Lons, Lats)
    #dict_LonsCen['data'] = Lons
    #dict_LatsCen['data'] = Lats
    #'''


    dict_LonsCor = {'data':None,
                    'long_name':'Longitude of corner of pixels', 'standard_name':'pixels corner longitude',
                    'units':'degrees_east', 'undef':-999.99, 'axis':'YX'}
    dict_LatsCor = {'data':None,
                    'long_name':'Latitude of corner of pixels', 'standard_name':'pixels corner latitude',
                    'units':'degrees_north', 'undef':-999.99, 'axis':'YX'}
    ...
    Lons, Lats = np.meshgrid(Lons, Lats)
    dict_LonsCor['data'] = Lons
    dict_LatsCor['data'] = Lats

    return GOES(dict_LonsCor), GOES(dict_LatsCor);


#-----------------------------------------------------------------------------------------------------------------------------------

def accumulate_in_gribmap(lightning_lon, lightning_lat, Lons, Lats, dx=200, dy=200, dz=200, fmt=np.int32):
    ...
    Lons : ndarray
        2D array with gridmap corners longitudes.

    Lats : ndarray
        2D array with gridmap corners latitudes.
    ...
    fmt : dtype, optional
        The type of the returns. Default value is np.int32.


    Returns
    -------
    Density: object
        Lightning density acumulated over gridmap.
    ...
    try:
        ...
        try:
            ...
            dict_Field = {'data':None,
                          'long_name':LongName, 'standard_name':StandardName,
            ...
            print('    There are {:.0f} lightnings inside gribmap'.format(Ltng_Lon.shape[0]))
